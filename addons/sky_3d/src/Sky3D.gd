...
## Explore and configure the settings in the Sky3D, SunLight, MoonLight, [SkyDome], and [Sky3DTimeOfDay] nodes.
...
## The Sky3DTimeOfDay node.
var tod: Sky3DTimeOfDay
## The SkyDome node.
var sky: SkyDome
## The Sky shader.
...
## Allows time to progress in the editor. Alias for [member Sky3DTimeOfDay.editor_time_enabled].
...
## Allows time to progress in game. Alias for [member Sky3DTimeOfDay.game_time_enabled].
...
## A readable game date string, eg. '2025-01-01'. Alias for [member Sky3DTimeOfDay.game_date].
@export_custom(PROPERTY_HINT_NONE, "", PROPERTY_USAGE_EDITOR | PROPERTY_USAGE_READ_ONLY) 
...
## A readable game time string, e.g. '08:00:00'. Alias for [member Sky3DTimeOfDay.game_time].
@export_custom(PROPERTY_HINT_NONE, "", PROPERTY_USAGE_EDITOR | PROPERTY_USAGE_READ_ONLY) 
...
## Alias for [member Sky3DTimeOfDay.current_time].
...
## Alias for [member Sky3DTimeOfDay.minutes_per_day].
...
## Alias for [member Sky3DTimeOfDay.update_interval].
...
## Pauses time calculation. Alias for [method Sky3DTimeOfDay.pause].
func pause() -> void:
	if tod:
		tod.pause()


## Resumes time calculation. Alias for [method Sky3DTimeOfDay.resume].
func resume() -> void:
	if tod:
		tod.resume()
...
func _initialize() -> void:
	...
	if has_node("Sky3DTimeOfDay"):
		tod = $Sky3DTimeOfDay
	elif is_inside_tree():
		tod = Sky3DTimeOfDay.new()
		tod.name = "Sky3DTimeOfDay"
		add_child(tod, true)
